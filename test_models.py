...
result_root = osp.join(args.result,args.modality+".pth")
for i, (path,data, label) in data_gen:
    if i >= max_num:
        break
    key_word = (path[0].split("/")[-1])

    rst = eval_video((i, data, label))
    output.append(rst[1:])
    video_pred = np.mean(rst[1], axis=0)
    video_labels = rst[2].cpu().numpy()
    save_dict[key_word] ={"pred":video_pred,"label":video_labels}
...
torch.save(save_dict,result_root)
video_pred = [np.argmax(np.mean(x[0], axis=0)) for x in output]
video_labels = [x[1] for x in output]

cf = confusion_matrix(video_labels, video_pred).astype(float)
#cf 的行表示的是真值数，列表示为预测为第i 列的个数
#label 的总个数
